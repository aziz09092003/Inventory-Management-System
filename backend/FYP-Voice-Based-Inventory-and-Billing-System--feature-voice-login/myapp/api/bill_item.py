...
from myapp.schemas.bill_item import BillItemCreate, BillItemBatchCreate, BillItemRead
...
@router.post("/batch", response_model=List[BillItemRead], status_code=status.HTTP_201_CREATED)
async def create_bill_items_batch_endpoint(data: BillItemBatchCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await crud.create_bill_items_batch(db, [item.model_dump() for item in data.items], current_user)
